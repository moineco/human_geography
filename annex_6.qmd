...
```{r}
maps::map(database = "state", regions = "California")
```
...
```{r}
usa <- maps::map(database = "usa", plot = FALSE) # save map
...
maps::map(database = "state", regions = "California", col = "blue",
    fill = TRUE)
```

```{r}
## California as a purple state
maps::map(database = "state", regions = "California", col = cal.color,
fill = TRUE)
```
...
maps::map(database = "state") # create a map
